...
const continental = require('../../server/meals/continental');
...
describe('Continental Meals Data Shape', () => {
  const requiredFields = ['name', 'calories', 'proteinG', 'carbsG', 'fatG', 'estimated'];
  ...
  test('All meals should be objects, not strings', () => {
    const meals = getAllMeals(continental);
    meals.forEach((meal) => {
      expect(typeof meal).toBe('object');
      expect(meal).not.toBe(null);
    });
  ...
  test('Should have approximately 30+ meals across all categories', () => {
    const meals = getAllMeals(continental);
    expect(meals.length).toBeGreaterThanOrEqual(30);
  });
});
