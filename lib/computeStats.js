...
function computeStats(logs, profile) {
  ...
  const sorted = [...logs].sort((a, b) => {
    ...
    const aDate = typeof a.date === 'string' ? a.date : (a.date instanceof Date ? a.date.toISOString().slice(0, 10) : a.date);
    const bDate = typeof b.date === 'string' ? b.date : (b.date instanceof Date ? b.date.toISOString().slice(0, 10) : b.date);
